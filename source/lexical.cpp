    ...
    for(int nline = 0; nline < info->nlines; nline++)
    {
        ...
        while (CUR_POS != '\0')
        {
            ...
            if(isalpha(CUR_POS))  
            {
                ...
                {
                    int var_index = SearchVarName(str);
            ...
            {
                //printf("num %lg\n", strtod(CUR_STR, &CUR_STR));
                Node_t *new_node = CTOR_NUM(strtod(CUR_STR, &CUR_STR));
                new_node->info.nline = nline;
                new_node->info.cur_pos = CUR_STR;
                PUSH_TOK(stk, new_node);
            }
...
void DumpTokens (TokStack_t *stk, int line)
{
    ...
    sprintf(dot_cmd, "dot \"graph.txt\" -T png -o treepng/graph_tokens.png");
