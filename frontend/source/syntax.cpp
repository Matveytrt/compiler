...
Node_t *GetOperator(const TokStack_t *stk, int *stk_index)
{
    ...
    {
        node_left = GetVariable(stk, stk_index);

        node_left = GetAssign(stk, stk_index, node_left);
    }
    ...
    else if (IsOper(_TOKEN_, OP_RET) || IsOper(_TOKEN_, OP_BREAK))
    {
        node_left = _TOKEN_;
        (*stk_index)++;
        Node_t *node_left_left = GetLogic(stk, stk_index);
        node_left = NewNode(node_left, node_left_left, NULL);
    }

    else
    {
        node_left = GetLogic(stk, stk_index);
    }
...
Node_t *GetAssign(const TokStack_t *stk, int *stk_index, Node_t *node_left)
{
    assert(stk);
    assert(stk_index);
    assert(node_left);
    ...
    Node_t *node = _TOKEN_;
    (*stk_index)++;
    Node_t *node_right = GetLogic(stk, stk_index);
...
Node_t *GetLogic(const TokStack_t *stk, int *stk_index)
{
    assert(stk);
    assert(stk_index);

    Node_t *node = GetExpression(stk, stk_index);
    ...
    ReadInfo("Чтение операции мат логики");

    while (IsLogicOp(_TOKEN_))
    {
...
Node_t *GetFunction(const TokStack_t *stk, int *stk_index)
{
    ...
    Node_t *node_args = GetLogic(stk, stk_index);
